...
module.exports = async (conn, m, commands, chatUpdate) => {
    try {
        ...
        let metadata = isGroup ? await conn.groupMetadata(from) : {}
        ...
        let isBotAdmin = isGroup ? groupAdmin.includes(conn.user?.jid) : false
        let isAdmin = isGroup ? groupAdmin.includes(sender) : false
        let isOwner = [conn.user?.jid, ...config.owner].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(sender)
        ...
        user.expiredCheck(conn, m, _user);
        ...
        if (isMedia && m.msg.fileSha256 && (m.msg.fileSha256.toString("hex") in global.db.sticker)) {
            ...
            messages.key.fromMe = await areJidsSameUser(m.sender, conn.user.jid)
        ...
        if (isAntidelete && m.message && m.message.protocolMessage && m.message.protocolMessage.type == 0) {
            ...
            let msg = await conn.serializeM(await Store.loadMessage(key.remoteJid, key.id))
            ...
            conn.relayMessage(m.from, msg.message, { messageId: msg.id })
            await conn.sendText(m.from, tekss, msg, { mentions: [msg.sender] })
        }
