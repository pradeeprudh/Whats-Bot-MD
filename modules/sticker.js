...
ezio.addCommand(
  ...
  async (message, client) => {
    try {
      if (!quoted) { global.catchError = true; return await client.sendErrorMessage( message.from, 'Reply to Supported media With Caption', message.key, message ); }
      if (/image|video|sticker/.test(message.client.mime)) {
        ...
        if (isUrl(message.client.text))
          client.sendFile(message.from, isUrl(message.client.text)[0], "", message, { asSticker: true, author: ezio.config.exif.author, packname: ezio.config.exif.packname, categories: ["😄", "😊"],   });
        else { global.catchError = true; return await client.sendErrorMessage( message.from, 'No Url Match', message.key, message ); }
      } else if (message.client.text) {
        let fetch = await fetchUrl(global.api( "zenz", "/searching/stickersearch", { query: message.client.text }, "apikey" ));
        ...
        let download = await client.downloadMediaMessage(_message);
        client.sendFile(message.from, download, "", message, { asSticker: true, author: ezio.config.exif.author, packname: ezio.config.exif.packname, categories: ["😄", "😊"], });
      } else { global.catchError = true; return await client.sendErrorMessage( message.from, 'Reply to Supported media With Caption', message.key, message ); }
