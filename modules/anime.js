...
//         await axios.get(`${ezio.config.api.waifu}/sfw/${category}`)
//         .then(async (res) => {
//             message.client.command == `anime-${category}`
//             ? await client.sendFile(message.from, res.url, "", message, { asSticker: true, author: ezio.config.exif.author, packname: ezio.config.exif.packname, categories: ["😄", "😊"]})
//             : await client.sendMessage( message.from, { video: { url: res.url }, caption: ezio.config.exif.footer, gifPlayback: true }, { quoted: message } );
...
//         await axios.get(`${ezio.config.api.waifu}/nsfw/${category}`)
//         .then(async (res) => {
//             message.client.command == `anime-${category}`
//             ? await client.sendFile(message.from, res.url, "", message, { asSticker: true, author: ezio.config.exif.author, packname: ezio.config.exif.packname, categories: ["😄", "😊"]})
//             : await client.sendMessage( message.from, { video: { url: res.url }, caption: ezio.config.exif.footer, gifPlayback: true }, { quoted: message } );
...
ezio.addCommand( { pattern: [`anime`, `anime-gif`], sucReact: "🤹‍♀️", category: ["create",], }, async (message, client) => {
    await axios.get(`${ezio.config.api.waifu}/sfw/kill`)
    .then(async (res) => {
        message.client.command == `anime`
        ? await client.sendFile(message.from, res.url, "", message, { asSticker: true, author: ezio.config.exif.author, packname: ezio.config.exif.packname, categories: ["😄", "😊"]})
        : await client.sendMessage( message.from, { video: { url: res.url }, caption: ezio.config.exif.footer, gifPlayback: true }, { quoted: message } );
        global.catchError = false;
    }).catch(async (err) => { global.catchError = true; await client.sendErrorMessage(message.from,ezio.errorMessage(err),message.key,message); });
