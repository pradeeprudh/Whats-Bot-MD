 ...
 */
async function yt(url, quality = '360p', type, bitrate, server = "en68") {
  ...
  let filesize; 
  try { filesize = list[quality];}
  catch (e) { console.log('from lib2 : ' + e); filesize = list[Object.keys(list)[0]] }
...
async function getAudio(url) {
  try {
    return yt(url, '128kbps', "mp3", '128', servers[0]);
  } catch (e) {
    console.log("from lib: " + e);
    try {
      return yt(url, '128kbps', "mp3", '128', servers[1]);
    } catch (e1) {
      console.log("from lib: " + e1);
      try {
        return yt(url, "128kbps", "mp3", "128", servers[2]);
      } catch (e2) {
        console.log("from lib: " + e2);
        try {
          return yt(url, "128kbps", "mp3", "128", servers[3]);
        } catch (e3) {
          console.log("from lib: " + e3);
          try {
            return yt(url, "128kbps", "mp3", "128", servers[4]);
          } catch (e4) {
            console.log("from lib: " + e4);
            try {
              return yt(url, "128kbps", "mp3", "128", servers[5]);
            } catch (e5) {
              console.log("from lib: " + e5);
              return undefined;
            }
          }
        }
...
async function getVideo(url, quality = '360p') {
  try {
    return yt(
      ...
      quality,
      "mp3",
      quality.endsWith("p") ? quality.replace(/p/g, "") : quality,
      servers[0]
    );
  } catch (e) {
    console.log("from lib: " + e);
