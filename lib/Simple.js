...
class WAConnection {
    ...
    async sendFile(jid, PATH, fileName, quoted = {}, options = {}) {
        ...
        if (options.asSticker || /webp/.test(mime)) {
            ...
            pathFile = await writeExif(media, { packname: options.packname ? options.packname : ezio.config.exif.packname, author: options.author ? options.author : ezio.config.exif.author, categories: options.categories ? options.categories : [] })
...
class serialize {
    ...
    _client (conn) {
        ...
        this.client.isCreator = [this.client.botNumber, ...global.owner].map((v) => v.replace(/[^0-9]/g, "") + "@s.whatsapp.net").includes(this.sender);
